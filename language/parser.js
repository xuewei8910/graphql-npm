...
var Parser = /*#__PURE__*/function () {
  ...
  _proto.parseOperationDefinition = function parseOperationDefinition(allowRequiredField) {
    var start = this._lexer.token;
  ...
  _proto.parseSelectionSet = function parseSelectionSet(allowRequiredField) {
    var start = this._lexer.token;
    var _allowRequiredField = allowRequiredField;

    var parseSelectionFn = function parseSelectionFn() {
      return this.parseSelection(_allowRequiredField);
    };

    return {
      kind: _kinds.Kind.SELECTION_SET,
      selections: this.many(_tokenKind.TokenKind.BRACE_L, parseSelectionFn, _tokenKind.TokenKind.BRACE_R),
      loc: this.loc(start)
    };
  }
  /**
  ...
  _proto.parseSelection = function parseSelection(allowRequiredField) {
    return this.peek(_tokenKind.TokenKind.SPREAD) ? this.parseFragment() : this.parseField(allowRequiredField);
  }
  /**
  ...
  _proto.parseField = function parseField(allowRequiredField) {
    var start = this._lexer.token;
    ...
    if (allowRequiredField && this.expectOptionalToken(_tokenKind.TokenKind.BANG)) {
      required = true;
    } else {
      required = false;
    }
    ...
    return {
      ...
      directives: this.parseDirectives(false),
      selectionSet: this.peek(_tokenKind.TokenKind.BRACE_L) ? this.parseSelectionSet(allowRequiredField) : undefined,
      loc: this.loc(start),
      required: required
    };
  }
